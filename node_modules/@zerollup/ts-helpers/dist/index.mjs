...
var winSepRegex = new RegExp(regExpEscape(sep), 'g');
var posixSepRegex = /\//g;
        ...
        for (var _i = 0, _a = this.tokenizers; _i < _a.length; _i++) {
            ...
            if (match) {
                return match.map(function (p) {
                    var newPath = relative(fileName, p.replace(posixSepRegex, sep)).replace(winSepRegex, '\/');
                    return isRelative(newPath) ? newPath : ('./' + newPath);
                });
            }
        }
        var defaultPath = relative(fileName, this.baseUrl + '/' + oldImport).replace(winSepRegex, '\/');
