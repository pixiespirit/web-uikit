...
export const includesDate = (dates: Array<Date> | undefined, date: Date): boolean => {
    return !!dates && dates.some((shouldBeHighlight) => isSameDate(shouldBeHighlight, date));
};

export const toCalendarMatrix = (dates: Array<Date>): Array<Array<Date | undefined>> => {
    if (dates.length === 0) {
        return [];
    }
    ...
    const calendarMatrix: Array<Array<Date | undefined>> = chunks<Date | undefined>(calendar, 7);
